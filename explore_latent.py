...
if __name__ == '__main__':
    ####PARAMETERS####
    dataset_name = 'h_scz_study' #'decomp_study_SA010'
    w_len = 40
    ...
    flat_outputs = torch.load('../bendr_ws/{}_{}_flat_output.pt'.format(dataset_name, w_len), map_location=device)
    rec_names = torch.load('../bendr_ws/{}_{}_rec.pt'.format(dataset_name, w_len), map_location=device)
    targets = torch.load('../bendr_ws/{}_{}_target.pt'.format(dataset_name, w_len), map_location=device)
    outputs = torch.load('../bendr_ws/{}_{}_output.pt'.format(dataset_name, w_len), map_location=device)
    ...
    if use_3d:
        ...
        ax.legend(loc='best')
        ax.set_title('UMAP latent space representation (of {}s segments) \ncolored by target class'.format(w_len), fontsize=8)
        ...
        for x in range(len(rec_names_0)):
            if (x + 1) == len(rec_names_0):
                ax.scatter(embedding[ids_0_per_record[x], 0], embedding[ids_0_per_record[x], 1], c='blue', label=class_names[0], alpha=0.5,
                           s=20)  # label=rec_names_0[x], alpha=0.5, s=20)
            else:
                ax.scatter(embedding[ids_0_per_record[x], 0], embedding[ids_0_per_record[x], 1], c='blue', alpha=0.5, s=20)
        for x in range(len(rec_names_1)):
            if (x + 1) == len(rec_names_1):
                ax.scatter(embedding[ids_1_per_record[x], 0], embedding[ids_1_per_record[x], 1], c='red', label=class_names[1], alpha=0.5, s=20)
            else:
                ax.scatter(embedding[ids_1_per_record[x], 0], embedding[ids_1_per_record[x], 1], c='red', alpha=0.5, s=20)
        ax.legend(loc='best')
        ax.set_title('UMAP latent space representation (of {}s segments) \ncolored by target class'.format(w_len),
                     fontsize=8)
        ...
        for x in range(len(rec_names_1)):
            if (x + 1) == len(rec_names_1):
                ax2.scatter(embedding[ids_1_per_record[x], 0], embedding[ids_1_per_record[x], 1], c='black', label=class_names[1], alpha=0.35, s=20)
            else:
                ax2.scatter(embedding[ids_1_per_record[x], 0], embedding[ids_1_per_record[x], 1], c='black', alpha=0.35, s=20)
        for x in range(len(rec_names_0)):
            ax2.scatter(embedding[ids_0_per_record[x], 0], embedding[ids_0_per_record[x], 1], c=colors[x], label=rec_names_0[x][6:], alpha=0.5, s=20)

    ax2.legend(fontsize=8)
    ax2.set_title(
        'UMAP latent space representation (of {}s segments)\n{} class colored and diferenciated \n& {} class all in black'.format(
            w_len, class_names[0], class_names[1]), fontsize=8)
    ...
    if use_3d:
        ...
        for x in range(len(rec_names_0)):
            if (x + 1) == len(rec_names_0):
                ax3.scatter(embedding[ids_0_per_record[x], 0], embedding[ids_0_per_record[x], 1], c='black', label=class_names[0], alpha=0.35, s=20)
            else:
                ax3.scatter(embedding[ids_0_per_record[x], 0], embedding[ids_0_per_record[x], 1], c='black', alpha=0.35, s=20)
        for x in range(len(rec_names_1)):
            ax3.scatter(embedding[ids_1_per_record[x], 0], embedding[ids_1_per_record[x], 1], c=colors[x], label=rec_names_1[x][6:], alpha=0.5, s=20)

    ax3.legend(fontsize=8)
    ax3.set_title(
        'UMAP latent space representation (of {}s segments)\n{} class colored and diferenciated \n& {} class all in black'.format(
            w_len, class_names[1], class_names[0]), fontsize=8)
    plt.show(block=False)
    a = 0
