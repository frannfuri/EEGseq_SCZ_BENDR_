...
if __name__ == '__main__':
    labels_path = '/home/jack/PycharmProjects/BENDR_datasets/labels/SA049_labels.csv'
    #################################
    ...
    new_col = []
    mean_2_SAPS = np.round(mean_2_SAPS, 4)
    for v in mean_2_SAPS:
        new_col.append(v)
    labels_info['real_and_pred_SAPS_continuous'] = new_col

    new_col = []
    mean_2_SAPS = np.round(min_max_simple_norm(mean_2_SAPS), 6)
    for v in mean_2_SAPS:
        new_col.append(v)
    labels_info['real_and_pred_SAPS_continuous_norm'] = new_col

    ax.set_title('Labeling of {}'.format(labels_path[-16:-11]), fontsize=10)
    labels_info.to_csv(labels_path[:-4]+'_regr.csv')
    plt.show(block=False)
    a = 0
    print('ready {}'.format(labels_path[-16:]))