...
if __name__ == '__main__':
    # PARAMETERS
    data_path = '../../BENDR_datasets/decomp_study_SA047'
    n_folds = 4
    model_path1 = '../linear-classifier-rslts_avp_pAug_bw_vpr_dp0307_f1f_th04_bcePw02_len40ov30_/best_model_f'
    model_path1_2 = '../linear-classifier-rslts_avp_pAug_pretOwn_vpr_dp0307_f1f_th04_bcePw02_len40ov30_/best_model_f'
    regression_task = False
    th = 0.4

    #################################
    model_path2 = '_{}_lr0.0001bs8.pt'.format(data_path.split('/')[-1])
    ...
    for f in range(n_folds):
        ...
        model_2 = LinearHeadBENDR_from_scratch(1, samples_len=samples_tlen * 256, n_chn=20,
                                             encoder_h=512, projection_head=False,
                                             # DROPOUTS
                                             enc_do=0.3, feat_do=0.7,  # enc_do=0.1, feat_do=0.4,
                                             pool_length=4,
        ...
        model_path_2 = '{}{}{}'.format(model_path1_2, f, model_path2)
        ...
        np.save('../pred_results_{}_bendrW.npy'.format(data_path[-5:]),
                [all_recs_predict_probabs, all_recs_predictions, all_recs_targets])
        # Predictions
        all_recs_predict_probabs_2 = []
        all_recs_predictions_2 = []
        all_recs_targets_2 = []
        for rec_i in valid_record_names:
            rec_predict_probabs = []
            rec_predictions = []
            ...
            for x, y, rec_name in validloader:
                if rec_name[0] == rec_i:
                    ...
                    rec_predictions.append(pred.item())
                    rec_targets.append(y.item())
            all_recs_predict_probabs_2.append(rec_predict_probabs)
            all_recs_predictions_2.append(rec_predictions)
            assert all_same(rec_targets)
            all_recs_targets_2.append(rec_targets)

        np.save('../preds_results/pred_results_{}_ownW.npy'.format(data_path[-5:]),
                [all_recs_predict_probabs_2, all_recs_predictions_2, all_recs_targets_2])
        ...
        for i in range(len(valid_record_names)):
            ...
            axs[i].set_yticks([0.25, 0.75], minor=True, fontsize=8)
            axs[i].set_yticks([0, 0.5, 1], minor=False, fontsize=8)
            ...
            if not regression_task:
                axs[i].plot(list(range(len(all_recs_predict_probabs[i]))), all_recs_predict_probabs[i],
                            label='probab.', linestyle='dashed', c='cornflowerblue', linewidth=0.8)
                axs[i].plot(list(range(len(all_recs_predict_probabs_2[i]))), all_recs_predict_probabs_2[i],
                            label='probab.', linestyle='dashed', c='lightcoral', linewidth=0.8)
            axs[i].scatter(list(range(len(all_recs_predictions[i]))), all_recs_predictions[i],
                           ...
                           c='blue', s=20)
            axs[i].scatter(list(range(len(all_recs_predictions_2[i]))), all_recs_predictions_2[i],
                           label='{}\n(target={})'.format(valid_record_names[i], int(all_recs_targets_2[i][0])),
                           c='red', s=20)
            axs[i].set_title('Sample predictions of best model CV it. n°{}'.format(f + 1), fontsize=8)
            axs[i].legend(fontsize=10, loc='center left', bbox_to_anchor=(0.8, -0.2))
            if not regression_task:
