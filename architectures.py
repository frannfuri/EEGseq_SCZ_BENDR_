...
class ConvEncoderBENDR_from_scratch(nn.Module):
    ...
    def load(self, filename, strict=True, device=None):
...
class DeepNet(nn.Module):
    def __init__(self, use_dropout=False, do1_p=0.5, do2_p=0.5):
        ...
        if use_dropout:
            ...
            self.do2 = nn.Dropout2d(do2_p)

    def forward(self, x):
        x = self.maxpool(F.leaky_relu(self.conv1(x)))
        if self.use_dropout:
            x = self.do1(self.maxpool(F.leaky_relu(self.conv2(x))))
            x = self.do2(self.avgpool1(F.leaky_relu(self.conv3(x))))
        else:
...
class BENDRClassification_from_scratch(nn.Module):
    ...
    def __init__(self, targets, samples_len=6 * 256, n_chn=20, encoder_h=512, contextualizer_hidden=3076,
        ...
        super(BENDRClassification_from_scratch, self).__init__()
        self.samples_len = samples_len
        self.n_chn = n_chn
        self.return_features = return_features
    ...
    def load_pretrained_modules(self, encoder_file, contextualizer_file, freeze_encoder=False,
                                ...
                                freeze_mask_replacement=True, strict=False, device=None):
        self.load_encoder(encoder_file, freeze=freeze_encoder, strict=strict, device=device)
        self.load_contextualizer(contextualizer_file, freeze=freeze_contextualizer, strict=strict, device=device)
